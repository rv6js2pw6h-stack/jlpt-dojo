...
(function () {
  ...
  const GRAMMAR = window.N5_GRAMMAR || [];
  const CATS = window.CATEGORIES || {};
  const TIERS = window.TIERS || {};
  const DAY = 86400000;
  const STORE_KEY = "n5dojo.v1";
  ...
  function renderDashboard() {
    ...
    $("#view-dashboard").innerHTML = `
      <div class="card pad hero">
        <div>
          <h1>${greet}&nbsp;! Prêt à dompter la grammaire&nbsp;<span class="jp">N5</span>&nbsp;?</h1>
          <p>${dueCount > 0
  ...
  function exportData() {
    ...
    a.download = "n5-dojo-sauvegarde-" + todayStr() + ".json";
    a.click();
