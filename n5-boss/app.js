...
(function () {
  ...
  /* =====================================================================
     ROUTEUR
     ===================================================================== */
  const VIEWS = ["dashboard", "practice", "flash", "challenge", "boss", "reference"];
  let current = "dashboard";
  function go(view) {
    if (current === "challenge") stopChallenge();
    if (current === "boss" && window.BossBattle) window.BossBattle.stop();
    current = view;
  ...
  function render(v) {
    ...
    else if (v === "boss") renderBoss();
    else if (v === "reference") renderReference();
  }

  /* ----------------------------- Boss Battle --------------------------- */
  function renderBoss() {
    if (!window.BossBattle) { $("#view-boss").innerHTML = '<p class="muted" style="padding:2rem;text-align:center">Boss module unavailable.</p>'; return; }
    window.BossBattle.mount($("#view-boss"), {
      grammar: GRAMMAR,
      byId,
      shuffle,
      esc,
      ...
      demoQs: DEMO_QS,
      buyLink: BUY_LINK,
      record: (id, correct) => { if (!isDemoSession) record(id, correct, correct ? 4 : 1, true); },
    });
  }
  ...
  function renderDashboard() {
    ...
    $("#view-dashboard").innerHTML = `
      ...
      <div class="boss-banner" data-go="boss" role="button" tabindex="0">
        <div class="bb-glow"></div>
        <div class="boss-banner-emoji"><span>鬼</span></div>
        <div class="boss-banner-txt">
          <div class="bb-kick">妖怪 BOSS BATTLE</div>
          <h3>Affronte les yokai</h3>
          <p>Réponds juste pour attaquer. Combos, coups critiques, 5 boss à terrasser.</p>
        </div>
        <div class="boss-banner-cta">Jouer →</div>
      </div>
  ...
  function buildBottomNav() {
    const items = [
      ...
      ["boss", "Boss", '<path d="M5 16l-2-9 5 4 4-7 4 7 5-4-2 9z"/><path d="M5 16h14v3H5z"/>'],
